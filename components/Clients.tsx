    ...
    <section className="bg-white w-full py-12 sm:py-20 overflow-hidden text-center px-3">
      ...
      >
        {CLIENT_NAMES.map((client, i) => (
          <span
            key={i}
            className="inline-block whitespace-nowrap hover:font-bold transition-all duration-300 px-1 sm:px-2"
          >
            {client}
            {i < CLIENT_NAMES.length - 1 && <span className="mx-1 sm:mx-2">•</span>}
          </span>
        ))}
      </motion.div>
        ...
        <div className="flex gap-6 sm:gap-14 items-center animate-marquee">
          {[...CLIENT_LOGOS, ...CLIENT_LOGOS].map((logo, i) => (
            <div
              key={i}
              className={`
                ${logo.size === "big" ? "w-24 h-14 sm:w-44 sm:h-24" : "w-16 h-10 sm:w-32 sm:h-16"}
